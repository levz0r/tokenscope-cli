...
7. **Sync to installed location** - After modifying `hooks/` or `lib/` files, copy them to `~/.claude/analytics/`:
   ```bash
   cp hooks/analytics-hook.sh ~/.claude/analytics/hooks/
   cp lib/*.sh ~/.claude/analytics/lib/
   ```
   This ensures the local development changes take effect immediately.
8. **Keep install.sh files in sync** - There are two install.sh files that must stay synchronized:
